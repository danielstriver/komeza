...
export default function HomeScreen() {
  ...
  const { language, currentRatings, entries } = state;
  const tr = t[language];
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ background: '#F7F4EF', paddingBottom: '100px' }}>
      {/* Header */}
      <div
        ...
        {CATEGORIES.map((cat, idx) => {
          ...
          return (
            <motion.div
              ...
              style={{
                background: '#fff',
                border: `1.5px solid ${selected ? cat.color + '35' : '#F0EBE3'}`,
                boxShadow: selected ? `0 4px 24px ${cat.color}18` : '0 1px 8px rgba(0,0,0,0.04)',
              }}
            >
              {/* Category header */}
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-sm" style={{ color: '#1C2B2B' }}>
                      {tr[cat.key]}
                    </p>
                    <p className="text-xs mt-0.5" style={{ color: '#B0A99F' }}>
                      {selected
                        ? `${selected}/5`
              ...
              <div className="flex items-center justify-between gap-2">
                <span className="text-[10px] w-14 leading-tight text-right" style={{ color: '#C0B8AF' }}>
                  {tr[cat.lowKey]}
                </span>
                  ...
                  {DOT_LABELS.map((val) => {
                    ...
                    return (
                      <motion.button
                        ...
                        style={{
                          background: isSelected ? cat.color : isFilled ? cat.color + '22' : '#F3F4F6',
                          color: isSelected ? '#fff' : isFilled ? cat.color : '#9CA3AF',
                ...
                <span className="text-[10px] w-14 leading-tight" style={{ color: '#C0B8AF' }}>
                  {tr[cat.highKey]}
                </span>
              </div>
            </motion.div>
      ...
      <div className="px-4 pt-6 pb-2">
        <motion.button
          ...
          style={{
            background: allRated ? 'linear-gradient(135deg, #1a4731, #2d6a4f)' : '#E5E7EB',
        ...
        {entries.length > 2 && (
          <button
            ...
            className="w-full mt-3 py-3 rounded-2xl text-sm font-medium transition-all"
